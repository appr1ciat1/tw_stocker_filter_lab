...
def main():
    ...
    # 持續掛榜警示參數：敏感度分析顯示 3×8×2 在「效果強度 vs 獨立股票數」最平衡
    ap.add_argument("--persist-rounds", type=int, default=3)
    ap.add_argument("--persist-per", type=int, default=8)
    ...
    # ── 供 paper 頁引用的統計摘要（頁面讀這份，避免把數字寫死而變過時）──
    def blk(name, sub):
        if len(sub) == 0:
            return None
        return dict(name=name, n=int(len(sub)), avg=float(sub["net"].mean()),
                    wr=float((sub["net"] > 0).mean()))
    # 持續掛榜（近 rounds×per 個訊號日，每輪皆 ≥min_hits 次）
    sd = sorted(pd.Timestamp(x) for x in sig["signal_date"].unique())
    R, P, M = args.persist_rounds, args.persist_per, args.persist_hits
    qmap = {}
    for d in sd:
        h = [x for x in sd if x <= d][-(R * P):]
        if len(h) < R * P:
            continue
        s = sig[sig["signal_date"].isin(h)]
        ch = [set(h[i * P:(i + 1) * P]) for i in range(R)]
        qmap[d] = {str(t) for t, g in s.groupby("ticker")
                   if all(g["signal_date"].isin(c).sum() >= M for c in ch)}
    # simulate() 存的是 date 物件、qmap 鍵是 Timestamp，需正規化後才對得上
    dq = done.copy()
    dq["sdnorm"] = pd.to_datetime(dq["signal_date"])  # 欄名不可底線開頭：itertuples 會改名
    dq = dq[dq["sdnorm"].isin(list(qmap))]
    if len(dq):
        dq = dq.assign(q=[str(r.ticker) in qmap.get(r.sdnorm, set())
                          for r in dq.itertuples()])
    stats = dict(
        as_of=str(sig["signal_date"].max().date()),
        window=f"{sig['signal_date'].min().date()} ~ {sig['signal_date'].max().date()}",
        persist_rule=f"{R}輪×{P}日×每輪≥{M}次",
        blocks=[b for b in [
            blk("全部訊號（基準）", done),
            blk("排名 #1", done[done["rank"] == 1]),
            blk("#1 且出現次數≥中位數", dd[(dd["rank"] == 1) & (dd["freq"] >= fr.median())]),
            blk("#1 且未持續掛榜", dq[(dq["rank"] == 1) & (~dq["q"])] if len(dq) else dq),
            blk("出現次數少（後1/3）", dd[dd["freq"] <= dd["freq"].quantile(1 / 3)]),
            blk("⚠️ 持續掛榜", dq[dq["q"]] if len(dq) else dq),
        ] if b],
